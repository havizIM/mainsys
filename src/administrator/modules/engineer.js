            ...
            let html = `
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <p class="p-20">
                            <label>Photo</label>
                            <img src="${data.photo === null ? `${SET.baseURL()}assets/images/detail.svg` : `${SET.apiURL()}engineer/file/${data.photo}`}" class="img-fluid" alt="">
                        </p>

                        <div class="form-group mt-5">
                            <a href="#/engineer/edit/${data.id}" class="btn btn-md btn-block btn-success">Edit</a>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="file" class="dropify" name="photo" id="photo" ${data.photo === null ? '' : `data-default-file="${SET.apiURL()}engineer/file/${data.photo}`}">
                                    </div>
...
const engineerController = ((SET, DT, UI, LU) => {
            ...
            submitHandler: form => {
                $.ajax({
                    ...
                    success: res => {
                        ...
                        location.hash = `#/engineer/${res.results.id}`
                    },
                    error: ({ responseJSON }) => {
    ...
    const _fetchEngineer = (TOKEN, id, callback) => {
        $.ajax({
            url: `${SET.apiURL()}engineer/${id}`,
            type: 'GET',
            dataType: 'JSON',
    ...
    const _detailObserver = (TOKEN, id, data) => {
        ...
        let observer = new MutationObserver(function (mutations, observer) {
            ...
            let engineerSchedule = []

            data.teams.forEach(v => {
                let bgClass;

                switch(v.schedule.type) {
                    case 'Preventive' : 
                        bgClass = 'bg-info'
                    break
                ...
                let obj = {
                    id: v.schedule.id,
                    title: v.schedule.building.building_name,
                    start: v.schedule.date + ' ' + v.schedule.time,
                    end: v.schedule.date + ' ' + v.schedule.time,
                    type: v.schedule.type,
                    className: bgClass
                }

                engineerSchedule.push(obj)
            })

            if (container.contains($('.btn-delete')[0])) {
                _openDelete('#main_content')
                _submitDelete(TOKEN, data => {
                    location.hash = '#/engineer'
                })
            }
                ...
                columns: [
                    {
                        ...
                        render: function (data, type, row) {
                            return `
                                <a href="#/engineer/${row.id}">${row.full_name}</a>
                            `;
                        }
                    },
                    {
                            ...
                            return `
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu animated flipInY" x-placement="bottom-start" style="position: absolute; transform: translate3d(-33px, 35px, 0px); top: 0px; left: 0px; will-change: transform;">
                                        ...
                                        <a class="dropdown-item" href="#/engineer/edit/${row.id}"><i class="fa fa-edit"></i> Edit</a>
                                    </div>
                                </div>
                            `;
                        }
...
})(settingController, dtController, engineerUI, lookupController)
