...
const profileController = ((SET, UI) => {
    ...
    const _fetchProfile = TOKEN => {
        $.ajax({
            ...
            success: ({ results }) => {
                $('#profile_photo').attr('src', results.partner_user.partner.logo === null ? `${SET.baseURL()}assets/images/user.png` : `${SET.apiURL()}partner/file/${results.partner_user.partner.logo}`)
                $('#profile_name').text(results.partner_user.full_name)
                $('#profile_position').text(results.partner_user.position)
                $('#profile_phone').text(SET.replaceNull(results.partner_user.phone))
                $('#profile_address').text(SET.replaceNull(results.partner_user.address))
                $('#profile_email').text(SET.replaceNull(results.email))

                $('#partner_name').text(SET.replaceNull(results.partner_user.partner.partner_name))
                $('#partner_phone').text(SET.replaceNull(results.partner_user.partner.phone))
                $('#partner_fax').text(SET.replaceNull(results.partner_user.partner.fax))
                $('#partner_handphone').text(SET.replaceNull(results.partner_user.partner.handphone))
                $('#partner_email').text(SET.replaceNull(results.partner_user.partner.email))
                $('#partner_address').text(SET.replaceNull(results.partner_user.partner.address))
                $('#partner_city').text(SET.replaceNull(results.partner_user.partner.city.city))
                $('#partner_province').text(SET.replaceNull(results.partner_user.partner.province.province))
